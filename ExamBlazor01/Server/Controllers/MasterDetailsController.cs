...
namespace work_001.Server.Controllers
{
    ...
    public class MasterDetailsController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromForm] CustomerVM customerVM)
        {
            if (ModelState.IsValid)
            {
                Customer customer = new Customer()
                {
                    ...
                    MaritalStatus=customerVM.MaritalStatus
                };
                ...
                {
                    foreach (var spot in customerVM.SpotList)
                    {
        ...
        public async Task<IActionResult> Update([FromForm] CustomerVM customerVM)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                customer.MaritalStatus = customerVM.MaritalStatus;
        ...
        public async Task<IActionResult> DeleteCustomer(int id)
        {
            ...
            var existBookings = _context.BookingEntries.Where(x => x.CustomerId == customer.CustomerId).ToList();
            if(existBookings is not null)
            {
                foreach (var entry in existBookings)
                {
                    _context.Remove(entry);
                }
            }
